        ...
        <el-table-column label="佣金费率配置" align="center" prop="commissionConfig">
          <template #default="scope">
            <template v-if="scope.row.commissionConfig">
              <el-tag
                v-for="(item, idx) in parseCommissionConfig(scope.row.commissionConfig)"
                :key="idx"
                size="small"
                type="info"
                class="mr-1 mb-1"
              >
                {{ (Number(item.commissionRate) * 100).toFixed(1) }}%
                ({{ item.effectiveTime ? item.effectiveTime.slice(0, 10) : '?' }}
                ~
      ...
      <div style="margin-top:8px">
        ...
        <el-table :data="commissionRows" border size="small">
          ...
          <el-table-column label="生效时间" width="195">
            <template #default="scope">
              <el-date-picker v-model="scope.row.effectiveTime" type="datetime" placeholder="请选择生效时间" value-format="YYYY-MM-DD HH:mm:ss" style="width:100%" />
            </template>
          </el-table-column>
          <el-table-column label="失效时间" width="195">
            <template #default="scope">
              <el-date-picker v-model="scope.row.expirationTime" type="datetime" placeholder="请选择失效时间" value-format="YYYY-MM-DD HH:mm:ss" style="width:100%" />
            </template>
          </el-table-column>
        ...
        <div v-if="commissionRows.length === 0" style="text-align:center;color:#999;padding:16px">
          请点击「新增一行」添加费率配置
        </div>
      </div>
