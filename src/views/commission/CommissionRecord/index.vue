  ...
  <div class="p-2">
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" border :data="CommissionRecordList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="佣金计算基数" align="center" prop="commissionBase" />
        <!-- <el-table-column label="业务员ID" align="center" prop="salesUserId" /> -->
        <el-table-column label="业务员实发金额" align="center" prop="salesAmount" />
        <!-- <el-table-column label="团队长ID" align="center" prop="teamUserId" /> -->
        <el-table-column label="团队长实发金额" align="center" prop="teamAmount" />
        <!-- <el-table-column label="总负责人ID" align="center" prop="projectUserId" /> -->
        <el-table-column label="总负责人实发金额" align="center" prop="projectAmount" />
        ...
        <!-- <el-table-column label="流水状态" align="center" prop="status">
          <template #default="scope">
...
import { listCommissionRecord, getCommissionRecord, delCommissionRecord, addCommissionRecord, updateCommissionRecord } from '@/api/commission/CommissionRecord';
...
const { insurance_policy_status, commission_calc_strategy } = toRefs<any>(proxy?.useDict('insurance_policy_status', 'commission_calc_strategy'));
...
const data = reactive<PageData<CommissionRecordForm, CommissionRecordQuery>>({
  ...
  rules: {
    ...
    policyId: [
      { required: true, message: "关联保单ID不能为空", trigger: "blur" }
    ],
    policyNo: [
    ...
    commissionBase: [
      { required: true, message: "佣金计算基数不能为空", trigger: "blur" }
    ],
    salesUserId: [
      { required: true, message: "业务员ID不能为空", trigger: "blur" }
    ],
    salesAmount: [
      { required: true, message: "业务员实发金额不能为空", trigger: "blur" }
    ],
    teamUserId: [
      { required: true, message: "团队长ID不能为空", trigger: "blur" }
    ],
    teamAmount: [
      { required: true, message: "团队长实发金额不能为空", trigger: "blur" }
    ],
    projectUserId: [
      { required: true, message: "总负责人ID不能为空", trigger: "blur" }
    ],
    projectAmount: [
      { required: true, message: "总负责人实发金额不能为空", trigger: "blur" }
    ],
    calcStrategy: [
      { required: true, message: "算账依据不能为空", trigger: "change" }
    ],
    status: [
      { required: true, message: "流水状态不能为空", trigger: "change" }
    ],
    salesRatio: [
      { required: true, message: "业务佣金比例不能为空", trigger: "blur" }
    ],
    teamRatio: [
      { required: true, message: "团队佣金比例不能为空", trigger: "blur" }
    ],
    projectRatio: [
      { required: true, message: "总负责人佣金比例不能为空", trigger: "blur" }
    ],
    salesUserName: [
      { required: true, message: "业务员姓名不能为空", trigger: "blur" }
    ],
    teamUserName: [
      { required: true, message: "团队负责人姓名不能为空", trigger: "blur" }
    ],
    projectUserName: [
      { required: true, message: "总负责人姓名不能为空", trigger: "blur" }
    ]
  }
});
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('commission/CommissionRecord/export', {
    ...queryParams.value
